...
const upperCaseNames = people => people.map(n => n.name.toUpperCase())
...
const getApplicantEmails = applicants => applicants.filter(applicant => applicant.advancedJSCourse).filter(applicant => applicant.age >= 18).map(applicant => applicant.email)
...
const mergeObjects = arr => arr.reduce((output, current) => ({ ...output, ...current }), {})
